...
def get(self,request):
        ...
        if(len(student_id)<9 or user_id=="-1" or password=="-1" or len(password)<7  or user_id==0):
            return Response(user_error_serializer(user_error(success=-2,message="Syntax Error")) .data)

        if(api_login(user_id,student_id,password)<=0):
            return Response(user_error_serializer(user_error(success=-1,message="Invalid Request")) .data)
        else:
            return Response(friend_get_response_serializer(UserModel.objects.all(),many=True).data)
