...
public class db_handler extends SQLiteOpenHelper {
    ...
    private static final String KEY_TASK_CONSOMATION_ID="task_consomation_id";
    ...
    public void onCreateTable(SQLiteDatabase db){
        ...
        sql="CREATE TABLE IF NOT EXISTS " + TABLE_TASK_INFO + "("
                ...
                KEY_TASK_CONSOMATION_ID+" INTEGER"+
...
//        Log.d("medical_update_des", medical.getOperation_name());
    ...
    //Con                                                                             //change
    public int createCon(consommation_ con){
        SQLiteDatabase db=getWritableDatabase();
        ContentValues values=new ContentValues();
...
//        Log.d("get_returned_exp", returned_expense.getType());
    ...
    public int createTask(task_class task){
        ...
        values.put(KEY_TASK_CONSOMATION_ID,task.getCon_id());
    ...
    public task_class getTask(int task_id){
        ...
        if(cursor!=null){
            if(cursor.moveToFirst()){
                ...
                task.setCon_id(cursor.getInt(10));
                Log.d("task_user_id", String.valueOf(task.getUser_id()));
                Log.d("tasK_date", task.getDate());
            }
        }
    ...
    public task_class getTask_by_con(int con_id){
        SQLiteDatabase db=getReadableDatabase();
        sql="SELECT * FROM "+TABLE_TASK_INFO+" WHERE "+KEY_TASK_CONSOMATION_ID+"=?";
        Cursor cursor=db.rawQuery(sql,new String[]{String.valueOf(con_id)});
        task_class task=null;
        if(cursor!=null){
            if(cursor.moveToFirst()){
                task=new task_class();
    ...
    public void deletetask(task_class taskClass){
        SQLiteDatabase db=getWritableDatabase();
        db.delete(TABLE_TASK_INFO, KEY_TASK_ID+ "=?", new String[]{String.valueOf(taskClass.getTask_id())});
        Log.d("re_del_status", "deleted");
