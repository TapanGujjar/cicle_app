...
public class json_parsing {
    ...
    public int main_parse(JSONObject object, Context context,ProgressDialog dialog) {
        con = context;
        loading=dialog;
    ...
    public int main_parse_without_image(JSONObject object, Context context,ProgressDialog dialog) {
        con = context;
            ...
            for (i = 0; i < cicle_list.size() && keys != null; i++) {
                ...
                if (keys.has(String.valueOf(cicle_list.get(i).getCicle_id()))) {
                    ...
                    if (sql_key < 0) {
                        ...
                        Log.d("adding", "adding");
                        cicle_list.get(i).setSql_cicle_id(sql_key);
                        cicle_list.get(i).setFlag(4);
                        handler.updateCicle(cicle_list.get(i));
                    }
                }
    ...
    public image_class downloadfile_url(Context context,String url,image_class image){
        class DownloadFileFromURL extends AsyncTask<image_class,ProgressDialog, image_class> {
            Context context;
            ...
            protected image_class doInBackground(image_class... f_url) {
                ...
                try {
                    ...
                    Log.d("Error in url","Error in url");
                    return null;
...
//    public void downloadfile_url(Context context,String url,image_class image){
...
//                    URLConnection conection = url.openConnection();
//                    Log.d("android_studio", "In BaxkGround3");
//                    int server_response=((HttpURLConnection)url.openConnection()).getResponseCode();
//                    Log.d("Server_response",String.valueOf(server_response));
//                    conection.connect();
//                    String  filename=image.getName();
//                    // this will be useful so that you can show a tipical 0-100%
//                    // progress bar
...
//                // dismiss the dialog after the file was downloaded
//                File file_download=new File(image.getPath());
//
//                if(Integer.parseInt(progress)==100) {
//                    if(handler.getImageBYSQL(image.getSql_image_id())==null) {
//                        Log.d("post_execute_progress", progress);
//                        Log.d("post_save_path",image.getPath());
//                        handler.createImage(image);
//                    }
//                }
//            }
...
//        DownloadFileFromURL downloadFileFromURL=new DownloadFileFromURL(context,url,image);
//        try {
//            downloadFileFromURL.execute("hell").get();
//        } catch (InterruptedException e) {
//            e.printStackTrace();
//
//        } catch (ExecutionException e) {
